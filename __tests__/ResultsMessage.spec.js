...
describe('ResultsMessage', () => {
  ...
  const washingMachine = context(
    'putting one load of laundry in the washing machine',
  );
  ...
  const showering = context(
    'showering every day for a week or producing 1L of dairy milk',
  );
  const trainJourney = context(
    'a 4hr train journey or producing a block of cheese',
  );
  const beefSteak = context('producing a beef steak');
  const eurostar = context('a round trip to France using the Eurostar');
  const londonDublin = context(
    'a trip from London to Dublin by train/ferry or a flight from London to Leeds',
  );
  ...
  const bangladesh = context(
    'the annual per capita CO2 emissions for a Bangladeshi person',
  );
  const cambodia = context('the annual per capita CO2 emissions for Cambodia');
  const nicaragua = context(
    'the annual per capita CO2 emissions for Nicaragua',
  );
